...
| `transaction-filters.tsx` | Client | Filtre par catégorie (dropdown persistant au-dessus de la table) |
| `edit-transaction-dialog.tsx` | Client | Dialog d'édition (récurrent, date, mois budgétaire, compte, bucket) |
| `cancel-transaction-dialog.tsx` | Client | Dialog d'annulation avec note obligatoire |
| `bucket-selection-dialog.tsx` | Client | Dialog de sélection de bucket (comptes SAVINGS/INVESTMENT) |
...
`getTransactions()` et `getFormData()` construisent des objets plain explicitement pour convertir les montants (string en PG) en `number` via `toNumber()` avant passage aux Client Components.
